...
initialize_workspace()
meta = load_identity()
DEFAULT_AGENT_NAME = os.getenv("DEFAULT_AGENT_NAME", meta.get("name", "Postiz Agent"))
...
def agent_template(mcp_url: str = None, mcp_config: str = None, **kwargs):
    ...
    from postiz_agent.graph_config import TAG_PROMPTS, TAG_ENV_VARS
        ...
        try:
            ...
            if not os.path.isabs(config_path) and "/" not in config_path:
                ...
                try:
                    ...
                    pkg_res = files("postiz_agent") / config_path
                    if pkg_res.is_file():
            ...
            if os.path.exists(config_path):
                ...
                logger.info(
                    f"postiz-agent: Loaded {len(mcp_toolsets)} MCP servers from {config_path}"
                )
        except Exception as e:
            logger.error(
                f"postiz-agent: Failed to load MCP config {effective_mcp_config}: {e}"
            )
